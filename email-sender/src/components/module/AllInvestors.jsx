...
const columnOptions = [
  { key: "partnerEmail", label: "Partner Email", permanent: true },
  { key: "investorName", label: "Investor Name" },
  { key: "partnerName", label: "Partner Name" },
  { key: "fundType", label: "Fund Type" },
  { key: "fundStage", label: "Fund Stage" },
  { key: "website", label: "Website (If Available)" },
  { key: "fundFocusSectors", label: "Fund Focus (Sectors)" },
  { key: "portfolioCompanies", label: "Portfolio Companies" },
  { key: "location", label: "Location" },
  { key: "twitterLink", label: "Twitter Link" },
  { key: "linkedinLink", label: "Linkedin Link" },
  { key: "facebookLink", label: "Facebook Link" },
  { key: "numberOfInvestments", label: "Number Of Investments" },
  { key: "numberOfExits", label: "Number Of Exits" },
  { key: "fundDescription", label: "Fund Description" },
...
const AllInvestors = () => {
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true);
      await axios.delete(`http://localhost:5000/investors/${id}`);
  ...
  const handleOk = async () => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/investors/${selectedInvestor.id}`,
        values
      );
